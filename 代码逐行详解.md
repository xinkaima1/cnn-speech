...
- **用途**：第 49 行用 `glob.glob("data/**/*.wav")` 找出所有 `.wav` 文件
- **类比**：在文件管理器里搜索 `*.wav`

#### 第 2 行：`import random`
- **模块**：`random` 是标准库，提供随机数生成功能
- **用途**：第 51 行用 `random.Random(seed).shuffle(pairs)` 固定种子打乱数据
...
| 参数 | 值 | 含义 |
|---|---|---|
| `sample_rate` | 16000 | 每秒采样 16000 个点，RAVDESS 数据集的标准采样率 |
| `n_fft` | 1024 | 每次 FFT 分析的窗口长度（采样点数），越大频率分辨率越高 |
| `hop_length` | 125 | 相邻两帧的间隔，16000/125 = 128 帧/秒 |
| `n_mels` | 64 | Mel 滤波器组数量，决定频谱图的"高度" |
...
### 2.3 核心函数 `wav_to_logmel`（第 17-32 行）
...
```python
    # 前置修复：数据里有 5 条立体声文件 → 双声道取平均合成单声道。
    # 不修的话频谱图是 [2,64,128]，和单声道的 [1,64,128] 拼不进同一个 batch（报 stack 错）
    if wav.shape[0] > 1:
...
#### 第 36-37 行：`__init__` 初始化
- 接收文件列表 `[(路径, 标签), ...]`
- 保存到 `self.file_list`，供后续方法使用

#### 第 38-39 行：`__len__` 长度
- 返回数据集样本总数
- `DataLoader` 需要知道数据集大小才能工作

#### 第 40-44 行：`__getitem__` 取单个样本
- 接收索引 `idx`，返回 `(频谱图, 标签)`
- 第 42 行 `torchaudio.load(path)`：读取 `.wav` 文件
- 第 43 行 `wav_to_logmel(wav)`：波形转频谱图
- 第 44 行：标签转为 `int64` 张量（PyTorch 损失函数要求 `int64`）

---

### 2.5 数据划分函数 `make_loaders`（第 46-57 行）
...
**为什么用全局？** 在本文件中，`melspec` 被多个地方反复调用（`wav_to_logmel` 函数里、`__getitem__` 方法里），放在全局只**创建一次**，不用每次用到时都重新初始化。
...
RAVDESS 原始录音就是 16kHz。对已有文件"提高采样率"= 上采样，**不会凭空造出高频信息**（没新频率可加，只会内存翻倍、训练变慢）。所以只能在 `n_fft`/`hop_length` 上调整。
