<!-- 由同目录 cnn-guide-v1.html 清洗生成（2026-08-28）：剥离样式/脚本/交互组件，保留全部文字、代码、公式、表格与问答。交互图表与可勾选清单请看 HTML 原版。 -->

# CNN 在语音上的应用 从目标函数推导到收敛曲线的完整通关手册

作业指导书 v1（基线版） · 我教你做，代码你写
...
| --- | --- | --- | --- |
| LeNet 风格 CNN（本作业） | ② | **实测 63.5%**（2026-08-27 修复重采样后跑出；修复前 bug 版 46.2%，见 6B 节 Q12）。文献同类设置约 60–75%——已进入该区间 | **主线，必做** |
...
## SECTION 4 · 第四步：编程实现七步路线图

规则：**骨架、参数含义、验收标准我给；代码你写**。**每个 Step 标题都写明本步动哪个文件**（新建还是续写）。数据管道与模型各占一个文件（dataset.py / model.py）；训练主流程集中在 train.py 一个文件里——Step 5 写主体，Step 6 / 7 各往末尾追加一段，每追加一段重跑一次验收。每步末尾的"验收"是自检点——全绿再进下一步，返工成本最低。项目结构（三个 .py + 产出）：
...
│   └── train.py     # Step 5 创建 + Step 6/7 续写：训练 + 画图 + 终评一条龙
├── speech_cnn.pth   # train.py 产出：107 万参数值（~4 MB；.gitignore 忽略）
├── loss / acc / fig_confusion.png  # train.py 产出（报告证据）
└── cnn-guide/       # 本指导书
```

> **三个 .py 文件的关系：一条流水线（先建立这张图，再往下读）**

用做菜打比方：**dataset.py 是备菜，model.py 是图纸（锅的形状），train.py 是开火炒菜 + 摆盘上菜一条龙**——训练、画收敛曲线、终评都在 train.py 一个文件里按顺序完成。 ① **dataset.py（备菜）**——回答"磁盘上 1440 个 wav 怎么变成网络能吃的格式"。三个部件：`wav_to_logmel()` 把波形变成 64×128 频谱图（CNN 只认图）；`RavdessDataset` 类是"清单管理员"，谁来取第几条就现场读哪个 wav 并转换好递出去；`make_loaders()` 把数据打乱切成 1152 训练 + 288 测试，包装成自动按 32 条一批送货的 DataLoader。 ② **model.py（图纸）**——只定义网络长什么样（三个卷积块 + 分类头），它自己不读数据、不训练。单独运行它只做一件事：造 2 条假数据过一遍网络，验证图纸尺寸没画错。 ③ **train.py（炒菜 + 上菜）**——分三段：训练循环（Step 5 主体，40 轮）；训练刚结束先存盘 speech_cnn.pth（Step 5 结尾）；趁 history 字典还在内存里接着画两张收敛曲线（Step 6 追加）；最后在测试集上终评 + 出混淆矩阵（Step 7 追加）。一次运行，权重 + 两张曲线图 + 混淆矩阵三样产出全部落袋。 依赖关系（箭头 = 谁用谁的产出）：

```text
dataset.py ─(make_loaders)─┐
                           ├─→ train.py ─→ speech_cnn.pth + loss/acc.png + fig_confusion.png
model.py ───(SpeechCNN)────┘
```

依赖断了报 ImportError（train.py 用 import 搬前两个文件）——看到这个错，就知道断在哪条链上。所以执行顺序固定：**dataset → model → train**，train.py 按三段顺序续写，每追加一段就重跑一次验收。 **关键澄清（初学者第一大误区）：训练完的"菜"不是 model.py**。训练真正学到的是那 **1,073,032 个参数的具体数值**——model.py 只定义参数的"位置和形状"（"有个 3×3 的卷积核"），不含数值；训练前后 model.py 一个字都不变。类比：model.py 是**空白答题卡**，训练是往卡上填答案，训练好的模型是**填满答案的那张卡**。这张卡默认只活在显存里、程序一结束就随内存释放——想留住它，train.py 在训练循环刚结束处先存盘（菜装盘），后面要用时两行"复原"：

```python
# train.py 训练循环结束后（存盘）——已并入 Step 5 代码清单
torch.save(net.state_dict(), "speech_cnn.pth")   # 菜：107 万参数值

# 以后要用模型时两行"复原"：先照图纸造空模型，再把存好的数值灌回去
net = SpeechCNN()                                   # ① 造空模型（图纸）
net.load_state_dict(torch.load("speech_cnn.pth"))     # ② 灌入数值（答案）
```

三者关系一句话：**model.py 是图纸（永远不变），speech_cnn.pth 是炒好的菜（训练产物，约 4 MB），train.py 是炒菜过程（把图纸+数据变成那盘菜）**。所以 GitHub 仓库的灵魂是三个 .py——权重丢了，随时重新炒一盘（你的 4080 上只要 1~2 分钟）。
...
**事实**：RAVDESS 语音文件原生 **48 kHz / 16-bit / 单声道 / 每条约 4 秒**——读任何一条 wav 的文件头即可验证（文件第 25~28 字节按小端序读出来就是采样率；上面 Step 1 实测三条全是 48000）。本手册早先版本误写为"16 kHz"，是错的。 **bug 本体**：旧代码把 48k 波形直接喂给 `sample_rate=16000` 的 MelSpectrogram。这个参数**只负责告诉 Mel 滤波器组"每个 bin 对应多少 Hz"，不会重采样音频**。后果两条：① 频率刻度整体错位 3 倍（真实 3 kHz 的能量被记到 1 kHz 名下）；② hop=125 在 48k 波形上每秒产生 48000/125=**384 帧**而非设计的 128 帧 → 128 帧中心裁剪实际只保留 **0.33 秒**语音（原句约 4 秒）。铁证：一条 4.06 秒文件旧代码产出 4.06×384≈1559 帧，正是你 Step 2 验收时看到 [1, 64, 1346] 而非 [1, 64, 128] 的根源。 **为什么修法是"降采样到 16k"而不是"把参数升到 48k 档"**：语音能量几乎全在 8 kHz 以下（共振峰 F1~F4 在 300~4000 Hz），16 kHz 的奈奎斯特带宽刚好全覆盖；16 kHz 还是语音领域标准（wav2vec2 / HuBERT / emotion2vec 全按 16k 训练，以后做加分实验无缝衔接）。若留在 48k 档（n_fft=3072、hop=375 同比例放大）：计算量×3，且 Mel 轴上部约三分之一的频带浪费在 8~24 kHz 的近空频段上——纯亏。 **影响范围**：修复前的旧配置实测 46.18%（等效只喂 0.33 秒 + 频率错位）。2026-08-27 已按修正版代码完整重跑：test_acc **63.54%**（+17.4 个百分点），本手册各处数字均已同步为该轮实测——"发现 bug → 修复 → 精度 46.18% → 63.54%"这条因果链已是闭环的实测事实，不是预期。 **制度性教训（已固化）**：这个 bug 的根源不是手滑，是**没给数字分身份**——sample_rate=16000 被当成"惯例值随手写"，而它实际是管道一致性数字。第 1 章新增的"数字的三种身份"框（①数据事实要实测 / ②管道一致性要对账 / ③人为超参才可选）就是针对它的免疫机制，写任何数字前先归类。
...
本步**新建 src/train.py**——训练主程序：先写训练循环并存盘（本步全文如下），Step 6 / 7 再往这个文件末尾各追加一段。每个 batch 的训练五步，对应第 3 章推导的执行顺序（前向=式1/2/7，反向=式3–9，更新=式10）。注意：GPU 训练需要把模型和数据都搬到显存上：
...
> **验收**：第 1 个 epoch 结束 batch 平均 loss 应从 ≈2.08（=ln 8，随机初始化的理论值，报告里可写这个推理）明显下降；训练全程无报错；**训练结束后项目根目录出现一件产出：speech_cnn.pth**（`ls -lh speech_cnn.pth` 约 4 MB）。RTX 4080 上 1440 条 × 40 epoch 约 1–2 分钟（CPU 则需 10–20 分钟）。
...
### STEP 6 · 续写 src/train.py（末尾追加画图段）：收敛曲线（任务 (3) 的指定交付物）（≈ 30 分钟）

本步**把画图代码追加到 train.py 末尾**（接在 torch.save 之后）——训练时记进 history 字典的 40 轮数据还留在内存里，趁程序没结束直接画成两张 PNG（任务 (3) 的指定交付物，必须放进报告）。保存后**重新运行** `python src/train.py`（画图读的是内存里的 history，不重跑就没有数据可画）：
...
运行：`python src/train.py`（训练 + 存盘 + 画图一次全出）。判定标准见下一章的示意图——loss 应"快速下降 → 缓慢趋平"，test_acc 应上升后趋稳。

> **验收**：两张 PNG 生成在项目根目录；曲线形态符合"下降趋平"（若只出了 speech_cnn.pth 没出 PNG → 追加的画图段没接在文件最末尾、或缩进层级不对没被执行到——检查是否顶格写在 torch.save 之后）。若 loss 一路不动 → 检查 lr 与 zero_grad 是否遗漏；若 loss 震荡 → 减小 lr。
...
### STEP 7 · 续写 src/train.py（末尾再追加终评段）：有效性验证与混淆矩阵（≈ 30 分钟）

本步**把终评代码再追加到 train.py 末尾**（接在画图代码后面）——训练刚结束、模型还在显存里，直接在测试集上打分 + 出混淆矩阵（加分项，8×8 热图，看模型把谁认成谁——本轮实测混淆最重的是 fearful→happy 与 neutral→calm，见下方验收）。保存后再次运行 `python src/train.py`（训练 + 画图 + 终评一条龙，到此 train.py 全部写完）。"验证有效性"的最小充分论证 = **与随机基线对比**：

- 随机猜测基线：1/8 = **12.5%**。
- 你的模型（随机划分、GPU、40 epoch，2026-08-27 按本版流程实测）：**test accuracy = 63.54%**（288 条测试集对 183 条；train_acc 100%，最终 train_loss 0.0002、test_loss 2.28）。对比采样率 bug 修复前的旧配置 46.18%：重采样一项修复直接带来 **+17.4 个百分点**——报告里"发现 bug → 修复 → 精度从 46.18% 到 63.54%"就是现成的结果分析主线。
- 结论句式：「本模型测试准确率 63.54%，为随机基线（12.5%）的 5.1 倍，证明 CNN 能从频谱图中提取有效的情感判别特征；训练集 100% 与测试集 63.5% 的差距（36 个点）暴露了小数据 + 固定裁剪下的过拟合边界。」
...
> **验收**：终端打出 "test accuracy = 0.6354" 一句话结论 + fig_confusion.png 生成；能口头回答"哪两类最容易混淆、为什么"（本轮实测：fearful→happy 8 次、neutral→calm 6 次、sad→fearful / sad→disgust 各 5 次——高唤醒的 happy/fearful 互混、低唤醒的 neutral/calm/sad 互混，能量包络与音高特征相近所致；各类 recall：calm 0.81 / angry 0.78 / surprised 0.73 / disgust 0.70 / happy 0.57 / fearful 0.49 / neutral 0.50 / sad 0.44）。
...
**验收 = 拿一个你已经知道正确答案的小测试，去验证刚写完的代码**。它的价值在于**错误定位**：七个 Step 层层相扣（特征 → 数据管道 → 模型 → 训练），如果只在最后一步才第一次运行，出了错你根本不知道是哪层的问题——而每步验收过再进下一步，出问题时只需要检查最新的一层。返工成本从"全部重查"降到"只查一层"，这是它存在的全部理由。对报告还有一层意义：**每个验收的输出（打印的 shape、类的样本数、loss 数值）都是你报告"实验过程"章节的现成证据**——把它们连同终端输出截图或誊写进报告，比空口说"我实现了特征提取"可信得多。
...
| **Step 6 收敛曲线** 形态健康 | train.py 末尾追加画图段 → 重跑 python src/train.py → 内存里的 history 直接画两条线 → savefig。判读对照 5.1 节的表 | 图 A 原文放进报告（这是任务书第 (3) 问的直接交付物） |
| **Step 7 测试集** 最终成绩 | train.py 末尾再追加终评段 → 再次运行 python src/train.py → test_loader 上累计对错 → accuracy = 对的/总数；混淆矩阵用 sklearn：ConfusionMatrixDisplay.from_predictions → 存图 | "结果"小节：accuracy 数字 + 混淆矩阵图 + 5.2 节四步论证链 |
...
2. **结果**：test accuracy = 63.54%（288 条测试集对 183 条；附图 B 准确率曲线 + 混淆矩阵）。
3. **对比**：63.54% ≫ 12.5%（5.1 倍），且训练损失收敛（图 A：≈2.0 → 0.0002）排除了"碰巧"。
...
**一句话**：作业要求"选择一种**你能理解的**模型并给出导数求解过程"——wav2vec2 是近亿参数的 Transformer，其梯度推导远超本课范围；能被完整手推反向传播的最强结构，就是 CNN。**展开**：① 科学方法论上，先建 baseline 再谈 SOTA，我的 63.5%（基线，5.1 倍于随机）正是 84%（wav2vec2）和 91%（融合模型）这些数字的对照组，没有基线的"好成绩"无从谈起；② 技术谱系上，2014 路线与 2024 前沿共享同一地基——Mel 频谱、卷积、softmax 交叉熵，wav2vec2 的第一层至今仍是一个卷积特征编码器，作业里推的卷积梯度在那里原样生效；③ 我清楚前沿在哪（见 1.3 节谱系表），并可选做预训练 embedding 的对比实验。能在报告里画出这条演进线，本身就是对文献的理解深度。

================= 6B 代码级答辩拷问 =================

## SECTION 6B · 代码级答辩拷问（老师指着你的代码问）

第 6 章 9 题偏理论与推导；这一章全是"**train.py / model.py / dataset.py 里每一行为什么这么写**"。下方数字来自 **2026-08-27 按本版流程跑通的一轮完整训练**（40 epoch，RTX 4080，采样率修复后）——已是最终实测，直接背。每题一句话答案（背）+ 展开（理解）。只收关键点——工程细碎问题（import 放哪、变量名怎么起）不在答辩射程内，不背。

> **答辩数字速记卡（六个数必须脱口而出）**

初始 loss **≈2.0**（理论值 ln 8 = 2.079；终端 epoch 0 行的实测值在 1.9~2.0 一带）→ 最终 train_loss **0.0002**；train_acc **100%**；test_acc **63.54%**（= 随机基线的 5.1 倍；test_loss 2.28——远高于 train_loss，"自信地错"正是过拟合的数值签名）；随机基线 **12.5%**（=1/8）；参数量 **1,073,032**。前两个数证收敛，中间两个数证有效 + 暴露过拟合，最后两个数是对照锚点。

### 第一组：训练循环五步（train.py 的心脏）

**Q1：optimizer.zero_grad() 为什么不能删？删了会怎样？**

**一句话**：PyTorch 的梯度是"累加"进参数的 .grad 而不是覆盖——不清零，本 batch 的梯度会混进之前所有 batch 的，更新方向错，loss 卡在 2.08 附近不降。**展开**：五步顺序 = 擦黑板（zero_grad）→ 算梯度（backward）→ 用梯度更新（step）。累加式设计本是为"梯度累积"（显存不够时攒几批再一起更新）服务的；本作业每个 batch 独立更新，所以必须先清。实测 loss 从 ≈2.0 一路降到 0.0002，正是五步顺序正确的直接证据。
...
**Q3：为什么第 0 个 epoch 的 loss ≈ 2.08？实测比理论低一点，有问题吗？**

**一句话**：随机初始化的网络对 8 类输出近似均匀分布（各 ≈ 1/8），交叉熵 = −ln(1/8) = ln 8 = 2.079；实测略低（本轮终端 epoch 0 行在 1.9~2.0 一带，上一轮实测 1.9791）是因为初始化后输出已轻微偏斜，完全正常。**展开**：这个数不是巧合是必然——它同时验证了三件事：标签接对了、softmax 正常、loss 公式没错。报告加分句："初始 loss ≈ ln 8 = 2.08，与理论值吻合，验证了实现正确性。"
...
**Q11：train_acc 100% vs test_acc 63.54%，差 36 个点，是失败吗？**

**一句话**：过拟合——模型把训练集连同每个说话人的音色细节全"背"下来了，泛化到没见过的人只剩 63.5%。对这份作业不是失败：63.54% = 随机基线的 5.1 倍（方法有效），而"观察到过拟合 + 解释成因 + 给出改进"本身就是报告"结果分析"一节的完整科学叙事。**展开**：数值签名看两个 loss——train_loss 0.0002 vs test_loss 2.28，相差四个数量级：模型在测试集上"自信地错"（错误预测的 softmax 概率依然很高），CE 惩罚正是罚这种自信。早停点位看 acc.png 实测曲线：test_acc 升到 60% 一带后趋稳、不再系统性上升——追求最好单点结果就该停在趋稳段，还能省下白跑的十几个 epoch。

**Q12：为什么旧版只有 46.18%，不是文献常见的 60–75%？修了采样率 bug 后能到多少？**

**一句话**：46.18% 是**采样率 bug 版**的实测——RAVDESS 原生 48 kHz 被当 16 kHz 处理，频率刻度错位 3 倍 + 128 帧实际只含 0.33 秒语音（原句约 4 秒）；修复（T.Resample 降到 16k）后频率对齐、128 帧真正等于 1 秒。**已实测验证：2026-08-27 重跑 test_acc = 63.54%，+17.4 个百分点**，从"低于文献区间下限"直接进入文献常见区间 60–75%——瓶颈确实是它。剩余与文献上沿的差距来自固定 1 秒裁剪与无数据增强。**展开**：这是报告里最有力的一段叙事——"复盘发现输出帧数远超预期 → 读 wav 文件头确认 48 kHz → 定位到 sample_rate 参数只定刻度、不重采样音频 → 加 T.Resample 修复 → 精度从 46.18% 到 63.54%"。答辩被问"精度怎么不高"时，能讲出这条完整因果链，比无脑跑高分更显功力——工程里这叫 root cause analysis（根因分析）。有余力再做一版 target_frames=256（Linear 输入同步 8192→16384）喂 2 秒的对比实验。

**Q13：两张收敛曲线分别证明什么？合起来是什么？**

**一句话**：loss.png（≈2.0→0.0002，陡降后趋平）证明优化收敛、排除"碰巧"；acc.png（train 升至 100%、test 升至 63.5% 后趋稳）证明学到的特征有泛化价值。两图合起来 = 任务书第 (3) 问"验证有效性"的完整证据链。**展开**：判读口诀——loss 降而 acc 不升 = 在背训练集；acc 升而 loss 不降 = 疑似数据泄漏（查划分）；双双健康 = 本作业实际形态（有效学习 + 过拟合边界清晰）。

**Q14：混淆矩阵怎么读？看什么？**

**一句话**：8×8 方格，第 i 行第 j 列 = 真实类别 i 被预测成 j 的条数；主对角线越亮越好，非对角亮格 = 混淆点。**展开**：本轮实测最重的混淆是 fearful→happy（8 次）、neutral→calm（6 次）、sad→fearful / sad→disgust（各 5 次）——规律是**唤醒度相近的类互混**：高唤醒的 happy/fearful/angry 一团、低唤醒的 neutral/calm/sad 一团，能量包络与音高特征相近所致；文献常提的 happy↔surprised 在本轮反而不重（surprised recall 0.73 偏高）。报告结论句："模型错误集中在声学特征相近的类别，而非随机出错——说明卷积核学到的是有判别力的时频模式。"这句话直接呼应 Abdel-Hamid 2014 将 CNN 引入语音的原始论点[1]。
...
checkpoints/
EOF
# 上传黑名单：数据集、缓存、模型权重、.bak 参考答案——为什么必须先写它，见 8.7
...
python src/train.py              # 训练 + 存盘 + 收敛曲线 + 终评一条龙 → speech_cnn.pth + 三张 PNG（权重已被 git 忽略）
```
